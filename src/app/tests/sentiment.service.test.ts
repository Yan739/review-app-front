...
describe('ClientService', () => {
  ...
  beforeEach(() => {
    ...
    // On spy sur les méthodes http directement
    httpMock = {
  ...
  it('should fetch all clients', async () => {
    const clients = await firstValueFrom(service.getAll());
    expect(clients).toEqual(mockClients);
    expect(httpMock.get).toHaveBeenCalledWith('http://localhost:8080/api/client');
  });

  it('should create a client', async () => {
    const client = await firstValueFrom(service.create('charlie@test.com'));
  ...
  it('should update a client', async () => {
    await firstValueFrom(service.update(1, 'alice2@test.com'));
  ...
  it('should remove a client', async () => {
    await firstValueFrom(service.remove(2));
